...
AddReferralMethodRepository, FindReferralMethodRepository,
CheckReferralMethodExistByCodeRepository, CheckReferralMethodExistByUserIdRepository {
  ...
  public async checkByCode (code: string): Promise<boolean> {
    return !!(await this.dbClient.referralMethods.findFirst({
      where: { code }
    }))
  }

  public async checkByUserId (user_id: string): Promise<boolean> {
    return !!(await this.dbClient.referralMethods.findFirst({
      where: { user_id }
    }))
  }
